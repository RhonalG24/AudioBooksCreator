...
// const ssu = SpeechSynthesisUtterance
const synth = window.speechSynthesis


let text = "Hello everybody!!!!"
//utterThis.lang = 'es-ES'; //change language

startButton.addEventListener('click', startSpeech);
textArea.addEventListener('change', updateTextToRead);

function startSpeech(){
  updateTextToRead();
  const utterThis = new SpeechSynthesisUtterance(textToRead);
  utterThis.pitch = 1; //change pitch
...
const convertToText = async source => {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const text = content.items.reduce((acc, item) => (acc.str || acc) + item.str);
    contents.push(text);
  }
  return contents;
};
